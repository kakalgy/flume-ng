...
import com.flume.configuration.conf.channel.ChannelConfiguration;
import com.flume.configuration.conf.channel.ChannelType;
import com.flume.configuration.conf.sink.SinkConfiguration;
import com.flume.configuration.conf.sink.SinkGroupConfiguration;
import com.flume.configuration.conf.sink.SinkType;
import com.flume.configuration.conf.source.SourceConfiguration;
...
public class FlumeConfiguration {
	...
	public FlumeConfiguration(Properties properties) {
		...
		// Now iterate thru the agentContext and create agent configs and add
		// them
		// to agentConfigMap
	...
	private void validateConfiguration() {
		...
		while (it.hasNext()) {
			...
			if (!aconf.isValid()) {
				...
				errors.add(new FlumeConfigurationError(agentName, "", FlumeConfigurationErrorType.AGENT_CONFIGURATION_INVALID,
	...
	private boolean addRawProperty(String name, String value) {
		...
		if (name == null || value == null) {
			errors.add(new FlumeConfigurationError("", "", FlumeConfigurationErrorType.AGENT_NAME_MISSING, ErrorOrWarning.ERROR));
			return false;
		}

		// Empty values are not supported
		if (value.trim().length() == 0) {
			errors.add(new FlumeConfigurationError(name, "", FlumeConfigurationErrorType.PROPERTY_VALUE_NULL, ErrorOrWarning.ERROR));
			return false;
		}
		...
		if (index == -1) {
			errors.add(new FlumeConfigurationError(name, "", FlumeConfigurationErrorType.AGENT_NAME_MISSING, ErrorOrWarning.ERROR));
			return false;
		}
		...
		if (configKey.length() == 0) {
			errors.add(new FlumeConfigurationError(name, "", FlumeConfigurationErrorType.PROPERTY_NAME_NULL, ErrorOrWarning.ERROR));
			return false;
		}
	...
	public static class AgentConfiguration {
		...
		private boolean isValid() {
			...
			if (channelSet.size() == 0) {
				logger.warn("Agent configuration for '" + agentName + "' does not contain any valid channels. Marking it as invalid.");
				errorList.add(new FlumeConfigurationError(agentName, BasicConfigurationConstants.CONFIG_CHANNELS,
						FlumeConfigurationErrorType.PROPERTY_VALUE_NULL, ErrorOrWarning.ERROR));
				return false;
			}
			...
			if (sourceSet.size() == 0 && sinkSet.size() == 0) {
				logger.warn("Agent configuration for '" + agentName + "' has no sources or sinks. Will be marked invalid.");
				errorList.add(new FlumeConfigurationError(agentName, BasicConfigurationConstants.CONFIG_SOURCES,
						FlumeConfigurationErrorType.PROPERTY_VALUE_NULL, ErrorOrWarning.ERROR));
				errorList.add(new FlumeConfigurationError(agentName, BasicConfigurationConstants.CONFIG_SINKS,
						FlumeConfigurationErrorType.PROPERTY_VALUE_NULL, ErrorOrWarning.ERROR));
				return false;
			}
		...
		/**
		 * 获得组装Agent的配置信息(AgentName、Sources、Channels、Sinks)，key-value的形式
		 * 
		 * @return
		 */
		...
		/**
		 * 得到与type名字或者字符串(ChannelType)相同的Channel类型
		 * 
		 * @param type
		 * @return
		 */
		...
		private SinkType getKnownSink(String type) {
			...
			for (SinkType value : values) {
				...
				if (sink != null && sink.equalsIgnoreCase(type)) {
					return value;
				}
			}
			return null;
		...
		private SourceType getKnownSource(String type) {
			...
			for (SourceType value : values) {
				...
				String source = value.getSourceClassName();

				if (source != null && source.equalsIgnoreCase(type)) {
					return value;
				}
			}
			return null;
		 ...
		 * <ul>
		 * <li>-Yes: Get the ChannelType and set the string name of that to
		 * config and set configSpecified to true.</li>
		 * <li>-No.Look for config type for the given component:
		 * <ul>
		 * <li>-Config Found: Set config to the type mentioned, set
		 * configSpecified to true</li> -No Config found: Set config to OTHER,
		 * configSpecified to false, do basic validation. Leave the context in
		 * the contextMap to process later. Setting it to other returns a
		 * vanilla configuration(Source/Sink/Channel Configuration), which does
		 * basic syntactic validation. This object is not put into the map, so
		 * the context is retained which can be picked up - this is meant for
		 * older classes which don't implement ConfigurableComponent.</li>
		 * </ul>
		 * </li>
		 * </ul>
		 *
		...
		private Set<String> validateChannels(Set<String> channelSet) {
			...
			while (iter.hasNext()) {
				...
				if (channelContext != null) {
					...
					ChannelType chType = this.getKnownChannel(channelContext.getString(BasicConfigurationConstants.CONFIG_TYPE));
					boolean configSpecified = false;
					String config = null;
					// Not a known channel - cannot do specific validation to
					// this channel
					if (chType == null) {
					...
					try {
						conf = (ChannelConfiguration) ComponentConfigurationFactory.create(channelName, config, ComponentType.CHANNEL);
						logger.debug("Create Channel " + channelName);
						...
						// No need to add to error list - already added before
						// exception is
						// thrown
						if (conf != null)
							errorList.addAll(conf.getErrors());
						iter.remove();
						logger.warn("Could not configure channel " + channelName + " due to: " + e.getMessage(), e);
					}
				} else {
					iter.remove();
					this.errorList.add(new FlumeConfigurationError(agentName, channelName, FlumeConfigurationErrorType.CONFIG_ERROR,
							ErrorOrWarning.ERROR));
				}
			}

			this.channelContextMap = newContextMap;
			Set<String> tempChannelSet = new HashSet<>();
		...
		private Set<String> validateSources(Set<String> channelSet) {
			...
			if (sources == null || sources.isEmpty()) {
				logger.warn("Agent configuration for '" + agentName + "' has no sources.");
				errorList.add(new FlumeConfigurationError(agentName, BasicConfigurationConstants.CONFIG_SOURCES,
						FlumeConfigurationErrorType.PROPERTY_VALUE_NULL, ErrorOrWarning.WARNING));
			 ...
			 * The logic for the following code:
			 *
			 * Is it a known component? -Yes: Get the SourceType and set the
			 * string name of that to config and set configSpecified to true.
			 * -No.Look for config type for the given component: -Config Found:
			 * Set config to the type mentioned, set configSpecified to true -No
			 * Config found: Set config to OTHER, configSpecified to false, do
			 * basic validation. Leave the context in the contextMap to process
			 * later. Setting it to other returns a vanilla
			 * configuration(Source/Sink/Channel Configuration), which does
			 * basic syntactic validation. This object is not put into the map,
			 * so the context is retained which can be picked up - this is meant
			 * for older classes which don't implement ConfigurableComponent.
			 */
			while (iter.hasNext()) {
				...
				if (srcContext != null) {
					...
					if (srcType == null) {
						config = srcContext.getString(BasicConfigurationConstants.CONFIG_CONFIG);
						if (config == null || config.isEmpty()) {
							config = "OTHER";
						} else {
							configSpecified = true;
					...
					try {
						...
						srcConf = (SourceConfiguration) ComponentConfigurationFactory.create(sourceName, config, ComponentType.SOURCE);
						if (srcConf != null) {
							...
							srcContext.put(BasicConfigurationConstants.CONFIG_CHANNELS, this.getSpaceDelimitedList(channels));
						}
						...
						if (srcConf != null)
							errorList.addAll(srcConf.getErrors());
						iter.remove();
						logger.warn("Could not configure source  " + sourceName + " due to: " + e.getMessage(), e);
					}
				} else {
					iter.remove();
					errorList.add(new FlumeConfigurationError(agentName, sourceName, FlumeConfigurationErrorType.CONFIG_ERROR,
							ErrorOrWarning.ERROR));
		...
		private Set<String> validateSinks(Set<String> channelSet) {
			// Preconditions.checkArgument(channelSet != null &&
			// channelSet.size() >
			// 0);
			Map<String, Context> newContextMap = new HashMap<String, Context>();
			Set<String> sinkSet;
			...
			if (sinks == null || sinks.isEmpty()) {
				logger.warn("Agent configuration for '" + agentName + "' has no sinks.");
				errorList.add(new FlumeConfigurationError(agentName, BasicConfigurationConstants.CONFIG_SINKS,
						FlumeConfigurationErrorType.PROPERTY_VALUE_NULL, ErrorOrWarning.WARNING));
				return new HashSet<String>();
			} else {
			 ...
			 * The logic for the following code:
			 *
			 * Is it a known component? -Yes: Get the SinkType and set the
			 * string name of that to config and set configSpecified to true.
			 * -No.Look for config type for the given component: -Config Found:
			 * Set config to the type mentioned, set configSpecified to true -No
			 * Config found: Set config to OTHER, configSpecified to false, do
			...
			while (iter.hasNext()) {
				...
				if (sinkContext == null) {
					...
					errorList.add(new FlumeConfigurationError(agentName, sinkName, FlumeConfigurationErrorType.CONFIG_ERROR,
							ErrorOrWarning.ERROR));
				} else {
					String config = null;
					boolean configSpecified = false;
					...
					if (sinkType == null) {
						config = sinkContext.getString(BasicConfigurationConstants.CONFIG_CONFIG);
						if (config == null || config.isEmpty()) {
							config = "OTHER";
						} else {
							configSpecified = true;
					...
					try {
						...
						sinkConf = (SinkConfiguration) ComponentConfigurationFactory.create(sinkName, config, ComponentType.SINK);
						if (sinkConf != null) {
							sinkConf.configure(sinkContext);

						}
						if (!channelSet.contains(sinkConf.getChannel())) {
							throw new ConfigurationException("Channel " + sinkConf.getChannel() + " not in active set.");
						}
						...
						if (sinkConf != null)
							errorList.addAll(sinkConf.getErrors());
						logger.warn("Could not configure sink  " + sinkName + " due to: " + e.getMessage(), e);
					}
				}
		...
		/**
		 * Validates that each group has at least one sink, blocking other
		 * groups from acquiring it
		 *
		 * @param sinkSet
		 *            Set of valid sinks
		...
		private Set<String> validateGroups(Set<String> sinkSet) {
			...
			while (iter.hasNext()) {
				...
				if (context != null) {
					try {
						conf = (SinkGroupConfiguration) ComponentConfigurationFactory.create(sinkgroupName, "sinkgroup",
								ComponentType.SINKGROUP);
						...
						if (groupSinks != null && !groupSinks.isEmpty()) {
							...
							errorList.add(new FlumeConfigurationError(agentName, sinkgroupName, FlumeConfigurationErrorType.CONFIG_ERROR,
									ErrorOrWarning.ERROR));
							if (conf != null)
								errorList.addAll(conf.getErrors());
							throw new ConfigurationException(
									"No available sinks for sinkgroup: " + sinkgroupName + ". Sinkgroup will be removed");
						}
						...
						errorList.add(new FlumeConfigurationError(agentName, sinkgroupName, FlumeConfigurationErrorType.CONFIG_ERROR,
								ErrorOrWarning.ERROR));
						logger.warn("Could not configure sink group " + sinkgroupName + " due to: " + e.getMessage(), e);
					}
				} else {
					iter.remove();
					errorList.add(new FlumeConfigurationError(agentName, sinkgroupName, FlumeConfigurationErrorType.CONFIG_ERROR,
		...
		private Set<String> validGroupSinks(Set<String> sinkSet, Map<String, String> usedSinks, SinkGroupConfiguration groupConf) {
			...
			while (sinkIt.hasNext()) {
				...
				if (usedSinks.containsKey(curSink)) {
					logger.warn("Agent configuration for '" + agentName + "' sinkgroup '" + groupConf.getComponentName() + "' sink '"
							+ curSink + "' in use by " + "another group: '" + usedSinks.get(curSink) + "', sink not added");
					errorList.add(new FlumeConfigurationError(agentName, groupConf.getComponentName(),
							FlumeConfigurationErrorType.PROPERTY_PART_OF_ANOTHER_GROUP, ErrorOrWarning.ERROR));
					sinkIt.remove();
					continue;
				} else if (!sinkSet.contains(curSink)) {
					logger.warn("Agent configuration for '" + agentName + "' sinkgroup '" + groupConf.getComponentName()
							+ "' sink not found: '" + curSink + "',  sink not added");
					errorList.add(new FlumeConfigurationError(agentName, curSink, FlumeConfigurationErrorType.INVALID_PROPERTY,
							ErrorOrWarning.ERROR));
					sinkIt.remove();
					continue;
				} else {
		...
		private boolean addProperty(String key, String value) {
			...
			if (key.equals(BasicConfigurationConstants.CONFIG_SOURCES)) {
				if (sources == null) {
					...
					errorList.add(new FlumeConfigurationError(agentName, BasicConfigurationConstants.CONFIG_SOURCES,
							FlumeConfigurationErrorType.DUPLICATE_PROPERTY, ErrorOrWarning.ERROR));
					return false;
				}
			}
			...
			if (key.equals(BasicConfigurationConstants.CONFIG_SINKGROUPS)) {
				if (sinkgroups == null) {
					...
					errorList.add(new FlumeConfigurationError(agentName, BasicConfigurationConstants.CONFIG_SINKGROUPS,
							FlumeConfigurationErrorType.DUPLICATE_PROPERTY, ErrorOrWarning.ERROR));
					return false;
				}
			}

			ComponentNameAndConfigKey cnck = parseConfigKey(key, BasicConfigurationConstants.CONFIG_SOURCES_PREFIX);
			...
			cnck = parseConfigKey(key, BasicConfigurationConstants.CONFIG_CHANNELS_PREFIX);
			...
			cnck = parseConfigKey(key, BasicConfigurationConstants.CONFIG_SINKS_PREFIX);
			...
			errorList.add(new FlumeConfigurationError(agentName, key, FlumeConfigurationErrorType.INVALID_PROPERTY, ErrorOrWarning.ERROR));
			return false;
		}
		...
		private ComponentNameAndConfigKey parseConfigKey(String key, String prefix) {
			...
			// key must have a component name part after the prefix of the
			// format:
